...
from taxos.config import federal, payments as payment_config
from taxos.enums import PaymentDirection, PaymentMethod
from taxos.money import ZERO, cents, money, pct, positive
    ...
    *,
    ...
    if amount < ZERO:
        ...
        for key, entry in config["refund"].items():
            ...
            option.first_due_on = (
                today + timedelta(days=int(entry.get("typical_days", 0)))
            ).isoformat()
