...
from taxvault.auth.tokens import DEFAULT_TTL_SECONDS, TokenError, issue_token, read_token
from taxvault.crypto import CryptoError, normalise_ssn, seal_ssn, ssn_index
from taxvault.db.models import Account, AuditEvent, AuthChallenge, Taxpayer
...
def development_mode() -> bool:
    ...
    """
    if os.getenv("TAXVAULT_DEV_CODES", "").strip().lower() not in ("1", "true", "yes", "on"):
        return False
    url = os.getenv("TAXVAULT_DATABASE_URL", "")
    if url and not url.startswith("sqlite"):
        logger.warning("TAXVAULT_DEV_CODES ignored: a non-SQLite database is configured")
        return False
    return True
...
def verify_identity(
    ...
    *,
    ...
    if account.mobile_verified_at is None:
        raise AuthError(
            "Verify your mobile number first: we send a code to it and you enter the code."
        )
    # An omitted number means "the one already verified on this account". That is
    # not a weakening: the account's number was proven by a code, and re-typing a
    # number the server already holds proves nothing further.
    number = codes.normalise_mobile(mobile) if (mobile or "").strip() else (account.mobile_e164 or "")
    if number != (account.mobile_e164 or ""):
        raise AuthError(
            "That is not the mobile number verified on this account. Verify the new "
            "number first, then confirm your identity."
        )
