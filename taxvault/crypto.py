...
* **A master key** comes from the environment (`TAXVAULT_MASTER_KEY`), which in a
...
SCHEME = "v1"
_MASTER_ENV = "TAXVAULT_MASTER_KEY"
_KEY_BYTES = 32
...
# ---------------------------------------------------------------------------
def _home() -> Path:
    return Path(os.getenv("TAXVAULT_HOME", Path.home() / ".taxvault"))
...
def data_key(purpose: str) -> bytes:
    return _hkdf(master_key(), f"taxvault/{SCHEME}/{purpose}")
...
def decrypt_field(ciphertext: str, *, purpose: str, context: str = "") -> str:
    ...
    if len(parts) != 4 or parts[0] != SCHEME:
        raise CryptoError("not a taxvault ciphertext")
